...
import Coya, { FileSystemCoya, staticAssets } from 'coya-vue-component'
...
<template>
  <FileSystemCoya v-if="!activeConfig" class="demo-coya-main pt-10">
    <template #before-projects>
      <DemoCoyas class="pb-10" @select-config="activeConfig = $event" />
    </template>
  </FileSystemCoya>
  <Coya
  ...
  >
    ...
    <template #coya-block_1>
      ...
      <slider v-model="case1.numbs.val1" class="w-full pr-2 pl-2" :max="2000" :step="5" />
    </template>
    <template #coya-block_2>
      <p class="pl-5">
        Email:
      </p>
      <slider v-model="case1.numbs.val2" class="w-full pr-2 pl-2" :max="2000" :step="5" />
    </template>
    <template #coya-block_3>
      <p class="pl-5">
        Ad Networks:
      </p>
      <slider v-model="case1.numbs.val3" class="w-full pr-2 pl-2" :max="3000" :step="5" />
    </template>
    <template #coya-case2_1>
      <p class="pl-5">
        Value:
      </p>
      <slider v-model="case2.numbs.val1" class="w-full pr-2 pl-2" :max="60" />
    </template>
